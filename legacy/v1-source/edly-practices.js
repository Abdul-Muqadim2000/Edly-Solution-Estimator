/* Practices → platforms registry.
   Open edX is the only LIVE catalog: it comes from the master sheet (catalog-source.xlsx).
   Every other platform ships an INDICATIVE SAMPLE catalog — industry-benchmark scopes and
   hours so the tool is usable on day one, explicitly not Edly delivery records. Replace a
   sample by loading that practice's own sheet, or by adding solutions at the estimation desk. */
(function () {
  'use strict';

  const item = (cat) => ([id, name, desc, form, dep, f, r, acct]) => ({
    id, name, desc, form, status: 'Sample', deploy: dep,
    first: f, repeat: r != null ? r : Math.round(f * 0.35),
    build: null, saving: null, account: acct || null, integrations: null,
    notes: null, ref: null, category: cat, subCategory: 'Benchmark'
  });
  const B = (id, name, pitch, offer, cat, rows) => {
    const items = rows.map(item(cat));
    return { id, name, pitch, offerWhen: offer, featureCount: items.length,
      buildHrs: null, firstHrs: items.reduce((a, x) => a + x.first, 0),
      repeatHrs: items.reduce((a, x) => a + x.repeat, 0), saved: null,
      noEstimate: 0, inDev: 0, accounts: null, pairsWith: null, items };
  };
  const cat = (title, note, bundles) => {
    const all = [];
    bundles.forEach(b => b.items.forEach(i => all.push(i)));
    return {
      meta: {
        title, subtitle: note, compiled: 'benchmark', sample: true,
        totals: { features: all.length, buildHrs: null,
          firstHrs: all.reduce((a, x) => a + x.first, 0),
          repeatHrs: all.reduce((a, x) => a + x.repeat, 0),
          saved: null, noEstimate: 0, inDev: 0 },
        notes: [
  ...
  window.EDLY_PRACTICES = {
    /* practice → platforms. `live: true` means the catalog comes from the master sheet. */
    practices: [
      { id: 'edtech', name: 'EdTech / LMS', blurb: 'Learning platforms, course delivery and everything around them.', icon: '◫',
        platforms: [
      ...
      { id: 'appdev', name: 'App Development', blurb: 'Native and cross-platform mobile products.', icon: '▤',
        platforms: [
          { id: 'ios', name: 'iOS', catalog: ios },
          { id: 'android', name: 'Android', catalog: android },
          { id: 'crossplat', name: 'Cross-platform', catalog: cross }
        ] },
      { id: 'webdev', name: 'Web Development', blurb: 'Web applications, portals and content platforms.', icon: '▦',
        platforms: [
      ...
      { id: 'data', name: 'Data & Analytics', blurb: 'Lakehouse, warehouse and the reporting on top.', icon: '◨',
        platforms: [
      ...
      { id: 'aiml', name: 'AI / ML', blurb: 'Generative AI, vision models and the platform to run them.', icon: '◐',
        platforms: [
      ...
      { id: 'cloud', name: 'Cloud & DevOps', blurb: 'Landing zones, platforms and delivery pipelines.', icon: '◇',
        platforms: [
  ...
  /* flat lookup: platformId -> { practice, platform } */
  const flat = {};
  window.EDLY_PRACTICES.practices.forEach(p => p.platforms.forEach(pl => { flat[pl.id] = { practice: p, platform: pl }; }));
  window.EDLY_PRACTICES.find = id => flat[id] || null;
})();
