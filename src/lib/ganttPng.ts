...
export function ganttSvg(bars: readonly GanttBar[], weeks: number, cap: number): string {
  ...
  let out = `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" style="background:${color.surface}">`;
  out += `<text x="24" y="34" font-family="Arial" font-size="20" font-weight="bold" fill="${color.ink}">edly — Delivery timeline</text>`;
  out += `<text x="24" y="52" font-family="Arial" font-size="11" fill="${color.muted}">${esc(
    `${new Date().toDateString()} · team capacity ${cap} h/week · ${weeks} weeks`
  )}</text>`;

  for (let i = 0; i <= weeks; i += 1) {
    const x = LW + i * wpx;
    out += `<line x1="${x}" y1="${top}" x2="${x}" y2="${height - 30}" stroke="${color.gridLine}"/>`;
    if (i < weeks) {
      out += `<text x="${x + wpx / 2}" y="${top - 8}" font-family="Arial" font-size="10" fill="${color.chevron}" text-anchor="middle">W${i + 1}</text>`;
    }
  }
  ...
  bars.forEach((bar, index) => {
    ...
    out += `<text x="24" y="${y + 21}" font-family="Arial" font-size="11.5" fill="${color.inkSoft}">${esc(label)}</text>`;
    out += `<rect x="${LW + bar.start * wpx}" y="${y + 7}" width="${Math.max(3, bar.dur * wpx - 2)}" height="18" rx="5" fill="${
      bar.color
    }"${bar.span ? ' opacity="0.35"' : ''}/>`;
    out += `<text x="${LW + bar.start * wpx + 6}" y="${y + 20}" font-family="Arial" font-size="9.5" fill="${color.onSolid}" font-weight="bold">${Math.round(
      bar.hrs
    )}h</text>`;
  });

  out += `<text x="24" y="${height - 12}" font-family="Arial" font-size="9.5" fill="${color.faint}">Engineering estimate only · Edly by Arbisoft · edly.io</text></svg>`;
  return out;
}
