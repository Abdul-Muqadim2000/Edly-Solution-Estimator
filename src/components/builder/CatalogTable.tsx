...
/** Descriptions clamp to this until the row is expanded, so rows stay one scannable height. */
...
function Row({
  ...
  const detail: [string, string][] = [
    ...
    display.savings && item.saving !== null && item.saving !== undefined ? ['Reuse saving (source)', `${Math.round(item.saving * 100)}%`] : null,
    ...
    <div style={{ borderTop: `1px solid ${color.hairlineSoft}`, background: selected ? color.brandWashPale : color.surface, transition: 'background 110ms ease', ...hover.style }} {...hover.bind}>
      ...
      >
        ...
        <div style={{ minWidth: 0, paddingRight: 14 }}>
          ...
          <div style={{ fontSize: 12.5, color: color.muted, lineHeight: 1.5, paddingTop: 2 }}>
            {(item.desc ?? '').length > DESC_LIMIT && !expanded ? `${item.desc.slice(0, DESC_LIMIT).trim()}…` : item.desc}
          </div>
        </div>

        {!narrow ? (
          <div style={{ fontFamily: font.mono, fontSize: 11.5, color: color.muted }}>
            {!item.deploy || item.deploy === 'Not recorded' ? '—' : item.deploy}
          </div>
        ) : null}

        <div style={{ fontFamily: font.mono, fontSize: 13, fontWeight: 600, textAlign: 'right', color: color.ink }}>
          {item.first === null ? '—' : hours(item.first)}
        </div>

        <div style={{ textAlign: 'center', color: color.ghostCool, fontSize: 11, transform: expanded ? 'rotate(180deg)' : 'none', transition: 'transform 0.15s' }}>
          ▼
        </div>
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '7px 28px', maxWidth: 820 }}>
            {detail.map(([key, value]) => (
              <div key={key} style={{ display: 'flex', gap: 12, fontSize: 12.5, lineHeight: 1.55 }}>
                <span style={{ flex: '0 0 132px', color: color.ghostCool }}>{key}</span>
            ...
            {refUrl ? (
              <div style={{ display: 'flex', gap: 12, fontSize: 12.5, lineHeight: 1.55, gridColumn: '1 / -1' }}>
                ...
                <Link href={refUrl} style={{ minWidth: 0, wordBreak: 'break-all', color: color.brandDeep }} hover={{ color: color.red, textDecoration: 'underline' }}>
                  {refUrl}
                </Link>
              </div>
            ) : null}
